...
NSString * Botsford_Bryan_Project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation Botsford_Bryan_Project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_botsford_bryan_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Botsford_Bryan_Project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Botsford_Bryan_Project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_botsford_bryan_project2 = [[Botsford_Bryan_Project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_botsford_bryan_project2];
	
	NSString *_botsford_bryan_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _botsford_bryan_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_botsford_bryan_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Botsford_Bryan_Project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.botsford_bryan_project2.kroll" 
