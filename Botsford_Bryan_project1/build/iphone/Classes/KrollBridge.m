...
NSString * Botsford_Bryan_project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation Botsford_Bryan_project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_botsford_bryan_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Botsford_Bryan_project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Botsford_Bryan_project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_botsford_bryan_project1 = [[Botsford_Bryan_project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_botsford_bryan_project1];
	
	NSString *_botsford_bryan_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _botsford_bryan_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_botsford_bryan_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Botsford_Bryan_project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.botsford_bryan_project1.kroll" 
